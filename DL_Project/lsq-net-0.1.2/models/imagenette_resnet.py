...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, conv_layer=nn.Conv2d):
    """3x3 convolution with padding"""
    return conv_layer(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)


def conv1x1(in_planes, out_planes, stride=1, conv_layer=nn.Conv2d):
    """1x1 convolution"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, bit=4):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(inplanes, width,conv_layer=conv_layer)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation, conv_layer=conv_layer)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False,
        ...
        if self.bit <= 8:
            first_conv = partial(InputConv2dLSQ, bit=8)
            linear_layer = partial(LinearLSQ, bit=8)
        else:
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, stride, \
                        conv_layer=partial(Conv2dLSQ, bit=self.bit) if self.bit <= 8 else nn.Conv2d),
